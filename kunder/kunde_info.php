...
if( empty($kunde_navn) || empty($kunde_efternavn) || empty($kunde_adresse) || empty($kunde_telefonnummer) || empty($kunde_by) || empty($kunde_postnummer) ) {
  ...
  $stmt->bind_param('ssssisii', $kunde_navn, $kunde_efternavn, $kunde_adresse, $kunde_adresse_to, $kunde_telefonnummer, $kunde_by, $kunde_postnummer, $kunde_id_kunde);
  $stmt->execute();
  }
};
?>
...
if($_POST['kunde_adresse_to']) {
  $kunde_adresse_to = filter_input(INPUT_POST, 'kunde_adresse_to', FILTER_SANITIZE_STRING) or die('adresse to forkert');
  ...
  $sqlupdateinfo = ("UPDATE kunde_info SET kunde_navn=?, kunde_efternavn=?, kunde_adresse=?, kunde_adresse_to=?, kunde_telefonnummer=?, kunde_by=?, kunde_postnummer=? WHERE id_kunde_info ='$id_kunde_info'");
